...
import kotlinx.android.synthetic.main.cell_detail_home_anh_mot.*
...
class AdapterDetailHome(var listAnh: java.util.ArrayList<Int>, var activity: MainActivity) :
    ...
    fun onPushDataHuyet(dataHuyetDao: String?, idButton: Button?) {
        idButton?.setOnClickListener {
            ...
            Navigation.findNavController(it).navigate(R.id.phacDoHuyetFragment, bundleDataHuyet)
        }
    }
...
//        else if (listAnh[position] == 4) 4
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (listAnh[position] == 1) {
            onPushDataHuyet(idButton = holder.itemView.buttonDenHuyet1, dataHuyetDao = "Thừa mang")
//            onPushDataHuyet(idButton = buttonDenHuyetHinhMot6, dataHuyetDao = "Thừa mang")
//            onPushDataHuyet(idButton = buttonDenHuyet1, dataHuyetDao = "Thừa mang")
//            holder.itemView.anhThuNhat
        }
    }
}

class ViewHolderMot(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderHai(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderBa(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderBon(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderNam(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderSau(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderBay(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderTam(itemView: View) : RecyclerView.ViewHolder(itemView)
class ViewHolderChin(itemView: View) : RecyclerView.ViewHolder(itemView)
